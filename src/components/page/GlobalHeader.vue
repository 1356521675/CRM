<template>
  ...
  <a-layout-header
    v-if="!headerBarFixed"
    :class="[fixedHeader && 'top-ant-header-fixedHeader', sidebarOpened ? 'ant-header-side-opened' : 'ant-header-side-closed', 'top-ant-header']"
    :style="{ padding: '0' }"
  >
    <div
      v-if="mode === 'sidemenu'"
    ...
    >
      ...
      <a-icon
        ...
        @click="toggle"
      ></a-icon>
      <div class="logo-nav"  v-else>
        <logo class="toplogo"/>
        <HeadNav :target="target" />
      </div>
      <user-menu></user-menu>
    </div>
    <div
...
import HeadNav from '../tools/HeadNav'
