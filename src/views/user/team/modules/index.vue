          ...
          <s-table ref="table"
            ...
            <span slot="action"
              ...
              <template>
                ...
                <a-button class="btn-info"
                          size="small"
                          @click="move(record)"
                          v-show="moveBtnStatus"
                          style="margin-right:8px">部门转移</a-button>
                <a-button size="small"
                          style="margin-right:8px"
                          @click="openTips(record)"
                          class="btn-primary">关闭账户</a-button>
...
import editCustomer from "./userCenter";
import moveDep from "./movedep";
...
export default {
  ...
  created () {
    if (this.roles().permissionList.indexOf('98') != -1) {
      this.moveBtnStatus = true
    }
  },
  methods: {
    ...mapGetters(["roles"]),
    loadPage () {
      ...
      this.$refs.movedep.close()
      this.visible = true
      this.queryParam = {}
      this.$refs.table.refresh(true)
      this.$refs.tipsbox.cancleReview()
      if (this.roles().permissionList.indexOf('90') != -1) {
        this.moveBtnStatus = true
      }
    },
    openTips (record) {
