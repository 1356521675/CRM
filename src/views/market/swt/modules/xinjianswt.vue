<template>
  <div class="center-box">
    <h4 class="title">
      <span v-if="visible">新建商务通</span>
      <span v-else>编商务通</span>
    </h4>
    ...
    <div class="ant-tabs" v-show="visible">
      <div class="top">
        <a-form :form="sform" autocomplete="off" @submit="handleSearch">
          <a-row :gutter="16">
            <a-col :xs="24" :sm="24" :lg="{span:8,offset:0}">
              <a-form-item label="搜索" :colon="false">
                <a-input placeholder="请输入商务通名称" @change="checkSearch" v-decorator="['class_name']" />
              </a-form-item>
            </a-col>
            ...
            <a-col :xs="24" :sm="24" :lg="{span:9,offset:2}">
              <a-form-item label="状态筛选">
                <a-radio-group name="radioGroup" v-decorator="['class_id']" @change="showStatus">
      ...
      <div class="bottom">
        ...
        >
          <span slot="namebox" slot-scope="text, record">
            <template>
              <a-input
          ...
          <span slot="action" slot-scope="text, record">
            <template>
              <a-button class="btn-primary" size="small" @click="() => editSClass(record)">修改</a-button>
            </template>
          </span>
          <span slot="status" slot-scope="class_status">
            <template>
              <a-button
                v-if="class_status.value == 1"
          ...
          <span slot="openstatus" slot-scope="text, record">
            <template>
              <a-switch
                checkedChildren="开"
                unCheckedChildren="关"
                :checked="record.is_open.value == 1"
    ...
    <esource-class ref="esource" @ok="editSave" @close="editCancel" />
  </div>
</template>
...
export default {
  ...
  data() {
    return {
      ...
      loadData: parameter => {
        return getbscnymJoinList(Object.assign(parameter, this.queryParam))
          .then(res => {
  ...
  methods: {
    handleSearch(e) {
      ...
      this.sform.validateFields((err, values) => {
        this.queryParam.name = values.class_name;
    ...
    statusSwitch(record) {
      ...
      let parameter = {
        name: record.name,
        is_open: record.is_open.value == 1 ? 0 : 1,
        sort: record.sort,
        default: record.default.value,
        join_class: record.join_class
      };
      putbscnymJoinClass(record.bscnym_id, parameter)
        .then(res => {
          // _this.$refs.table.refresh(true);
          if (res.code == 200) {
            _this.$message.success("状态修改成功", 0.5);
            record.is_open.value = parameter.is_open;
          } else {
            _this.$message.warning(res.msg);
          }
        })
    ...
    showStatus(e) {
      ...
      if (this.openNum < 99) {
        this.queryParam.is_open = this.openNum;
      } else {
        this.queryParam.is_open = undefined;
      }
      this.$refs.table.refresh(true);
    },
    ...
    resortChange(record) {
      !record.saveModel && record.setNum && (record.sort = record.renumber);
    },
