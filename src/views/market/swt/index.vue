  ...
  <div class="mainbox">
    <div :class="['tab-content', {'phstyle': isMobile()}]">
      <a-tabs :activeKey="activeKey" :animated="false" @change="clickTab">
        <a-tab-pane key="1">
          <span slot="tab">
            <a-icon :component="marketxinlaiyuan" />商务通默认
          </span>
          <server-index ref="semSourcecon" />
        </a-tab-pane>
        <a-tab-pane key="2">
          <span slot="tab">
            <a-icon :component="marketfenlei" />分类关联
          </span>
          <server-new />
        </a-tab-pane>
        <a-tab-pane key="3">
          <span slot="tab">
            <a-icon :component="marketfenlei" />新建商务通
          </span>
          <xinjian-swt/>
        </a-tab-pane>
      </a-tabs>
...
import xinjianSwt from "./modules/xinjianswt";
export default {
  name: "service",
  components: {
    serverNew,
  ...
  methods: {
    clickTab(index) {
      ...
      if (index == 1) {
        sethistoryPage(1, 0);
        // this.$refs.semSourcecon.sourceClass();
      } else if (index == 2) {
        sethistoryPage(2, 0);
      }
    }
