<template>
  <div class="center-box">
    <h4 class="title">
      <span v-if="visible">新建分类</span>
      <span v-else>编辑分类</span>
    </h4>
        ...
        <a-form :form="sform" autocomplete="off" @submit="handleSearch">
          <a-row :gutter="16">
            <a-col :xs="24" :sm="24" :lg="{span:8,offset:0}">
              <a-form-item label="分类搜索" :colon="false">
                <a-input placeholder="请输入分类名称" @change="checkSearch" v-decorator="['class_name']" />
              </a-form-item>
            </a-col>
...
export default {
  ...
  data() {
    return {
      ...
      loadData: parameter => {
        return getClassification(Object.assign(parameter, this.queryParam))
          .then(res => {
  ...
  methods: {
    ...
    statusSwitch(record) {
      ...
      putClassification(parameter, record.class_id)
        .then(res => {
          // _this.$refs.table.refresh(true);
