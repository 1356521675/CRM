              ...
              <template>
                <table class="ant-table-content">
                  ...
                  <tr>
                    ...
                    <th style="border:0;">
                      <div>{{totalData.fullPayment_sum_total}}</div>
                    </th>
                    <th style="border:0;">
                      <div>{{totalData.upgrade_sum_total}}</div>
                    </th>
                    <th style="border:0;">
                      <div>{{totalData.upgrade_contracts_count_total}}</div>
                    </th>
                    <th style="border:0;">
                      <div>{{totalData.firstPayment_sum_total}}</div>
                    </th>
                    <th style="border:0;">
                      <div>{{totalData.balancePayment_sum_total}}</div>
                    </th>
                    <th style="border:0;">
                      <div>{{totalData.total_contracts_count_total}}</div>
                    </th>
                    <th style="border:0;">
                      <div>{{totalData.total_sum_total}}</div>
                    </th>
                    <th style="border:0;">
                      <div>{{totalData.upgrade_sum_rate_average}}%</div>
                    </th>
                    <th style="border:0;">
                      <div>{{totalData.upgrade_contracts_rate_average}}%</div>
                    </th>
                  </tr>
                </thead>
                </table>
...
import {
  getBiddingUpgradeList,
  getBiddingUpgradeTotal,
  getBiddingUpgradeDownload
} from "@/api/bidding";
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          title: "全款",
          dataIndex: "fullPayment_sum",
          width: "100px"
        },
        {
          title: "升级",
          dataIndex: "upgrade_sum",
          width: "100px"
        },
        {
          title: "升级单量",
          dataIndex: "upgrade_contracts_count",
          width: "100px"
        },
        {
          title: "首款",
          dataIndex: "firstPayment_sum",
          width: "100px"
        },
        {
          title: "尾款",
          dataIndex: "balancePayment_sum",
          width: "100px"
        },
        {
          title: "总成单量",
          dataIndex: "total_contracts_count",
          width: "100px"
        },
        {
          title: "总合同金额",
        ...
        {
          title: "升级业绩占比",
          dataIndex: "upgrade_sum_rate",
          scopedSlots: { customRender: "signRate" },
          width: "100px"
        },
        {
          title: "升级成单占比",
          dataIndex: "upgrade_contracts_rate",
      ...
      loadData: parameter => {
        if(this.searchStatus){
          return getBiddingUpgradeList(this.queryParam)
          .then(res => {
  ...
  methods: {
    ...
    getTotal(param) {
      getBiddingUpgradeTotal(param).then(res => {
    ...
    exportDatas() {
      ...
      let urls= api.abaseUrl+ '/bidding/upgrade-download?accessToken='+token+ this.downSrc
      window.open(urls)
    },
