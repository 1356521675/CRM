<template>
  <div class="center-box">
    <div class="show" v-show="visible">
      <div class="exportBox">
        <a-button :loading="downLoading" @click="exportDatas" class="btn-info">{{downInfo}}</a-button>
      </div>
      <a-form
                ...
                <table class="ant-table-content">
                  ...
                  <thead class="ant-table-thead" style="border:0;">
                    <tr>
                      ...
                      <th style="border:0;">
                        <div>{{totalData.allot_total}}</div>
                      </th>
                      <th style="border:0;">
                        <div>{{totalData.add_total}}</div>
                      </th>
                      <th style="border:0;">
                        <div>{{totalData.add_rate}}%</div>
                      </th>
                      <th style="border:0;">
                        <div>{{totalData.contract_total}}</div>
                      </th>
                      <th style="border:0;">
                        <div>{{totalData.contract_sum}}</div>
                      </th>
                      <th style="border:0;">
                        <div>{{totalData.contract_rate}}%</div>
                      </th>
                      <th style="border:0;">
                        <div>{{totalData.all_contract_count}}</div>
                      </th>
                      <th style="border:0;">
                        <div>{{totalData.all_contract_sum}}</div>
                      </th>
                      <th style="border:0;">
                        <div>{{totalData.all_contract_rate}}%</div>
                      </th>
                    </tr>
                  </thead>
                </table>
...
export default {
    ...
    return {
      ...
      loadData: parameter => {
        if (this.searchStatus) {
          return getBiddingPerList(Object.assign(parameter, this.queryParam))
            .then(res => {
  ...
  methods: {
    ...
    getTotal(param) {
      getBiddingPerTotal(param).then(res => {
    ...
    exportDatas() {
      ...
      let urls= api.abaseUrl+ '/bidding/per-all?accessToken='+token+ this.downSrc
      window.open(urls)
