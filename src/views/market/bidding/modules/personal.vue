              ...
              <template>
                <table class="ant-table-content">
                  ...
                  <tr>
                    ...
                    <th style="border:0;">
                      <div>{{totalData.total_achievement_sum_total}}</div>
                    </th>
                    <th style="border:0;">
                      <div>{{totalData.personal_contracts_count_total}}</div>
                    </th>
                    <th style="border:0;">
                      <div>{{totalData.personal_achievement_sum_total}}</div>
                    </th>
                    <th style="border:0;">
                      <div>{{totalData.personal_contracts_rate_average}}%</div>
                    </th>
                    <th style="border:0;">
                      <div>{{totalData.personal_achievement_rate_average}}%</div>
                    </th>
                  </tr>
                </thead>
                </table>
...
import {
  getBiddingPersonalList,
  getBiddingPersonalTotal,
  getBiddingPersonalDownload
} from "@/api/bidding";
...
export default {
  ...
  data() {
    return {
      ...
      columns: [
        ...
        {
          title: "总合同金额",
          dataIndex: "total_achievement_sum",
          width: "100px"
        },
        {
          title: "个人版单量",
          dataIndex: "personal_contracts_count",
          width: "100px"
        },
        {
          title: "个人版业绩",
          dataIndex: "personal_achievement_sum",
          width: "100px"
        },
        {
          title: "个人版单量占比",
          dataIndex: "personal_contracts_rate",
          scopedSlots: { customRender: "signRate" },
          width: "100px"
        },
        {
          title: "个人版业绩占比",
          dataIndex: "personal_achievement_rate",
      ...
      loadData: parameter => {
        if(this.searchStatus){
          return getBiddingPersonalList(this.queryParam)
          .then(res => {
  ...
  methods: {
    ...
    getTotal(param) {
      getBiddingPersonalTotal(param).then(res => {
    ...
    exportDatas() {
      ...
      let urls= api.abaseUrl+ '/bidding/personal-download?accessToken='+token+ this.downSrc
      window.open(urls)
    },
